...
test.describe("smoke", () => {
  ...
  test("recurring cards validate, persist, and create exactly one continuing occurrence on completion", async ({ page }) => {
    await openApp(page);
    await openWorkspace(page);
    await page.getByTestId("create-board").click();
    await page.getByTestId("text-dialog-input").fill("Recurring Board");
    await page.getByTestId("text-dialog-submit").click();
    await page.getByTestId("add-card-todo").click();
    await page.getByTestId("text-dialog-input").fill("Water plants");
    await page.getByTestId("text-dialog-submit").click();
    await page.getByTestId("recurrence-mode").selectOption("interval");
    await expect(page.getByTestId("recurrence-error")).toContainText("requires a due date");
    await page.getByTestId("recurrence-mode").selectOption("none");
    await expect(page.getByTestId("recurrence-interval")).toHaveCount(0);
    await page.getByTestId("recurrence-mode").selectOption("interval");
    await page.getByTestId("save-card").click();
    await expect(page.getByTestId("card-title-input")).toBeVisible();
    await page.getByTestId("card-due-input").fill("2099-01-31");
    await page.getByTestId("recurrence-interval").fill("1");
    await page.getByTestId("recurrence-unit").selectOption("month");
    await page.getByTestId("save-card").click();
    await page.getByTestId(/card-open-.*/).click();
    await expect(page.getByTestId("card-view-recurrence")).toContainText("Every 1 month");
    await page.getByTestId("card-view-complete").click();
    await expect.poll(async () => (await snapshot(page)).cards.length).toBe(2);
    let saved = await snapshot(page);
    expect(saved.cards.filter((file) => file.content.includes("recurrenceSourceId:")).length).toBe(1);
    expect(saved.cards.filter((file) => file.content.includes("completed: false")).length).toBe(1);
    await openApp(page, { reset: false });
    await expect.poll(async () => (await snapshot(page)).cards.length).toBe(2);
    const activeCard = page.locator(".task-card:not(.completed)", { hasText: "Water plants" }).first();
    await activeCard.click();
    await page.getByTestId("card-view-complete").click();
    await expect.poll(async () => (await snapshot(page)).cards.length).toBe(3);
    saved = await snapshot(page);
    expect(saved.cards.filter((file) => file.content.includes("recurrenceSourceId:")).length).toBe(2);
  });
});
